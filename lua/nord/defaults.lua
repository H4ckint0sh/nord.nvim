...
function defaluts.highlights()
  ...
  return {
    ...
    Cursor = { fg = c.snow_storm.origin, bg = utils.make_global_bg(options.transparent), reverse = true }, -- the character under the cursor
    CursorIM = { fg = c.snow_storm.brighter, bg = utils.make_global_bg(options.transparent), reverse = true }, -- like Cursor, but used when in IME mode
    ...
    ErrorMsg = { fg = c.snow_storm.origin, bg = utils.make_global_bg(options.transparent) }, -- error messages on the command line
    VertSplit = {
      fg = options.borders and c.polar_night.bright or c.polar_night.origin,
      bg = utils.make_global_bg(options.transparent),
    }, -- the column separating vertically split windows
    WinSeparator = {
      fg = options.borders and c.polar_night.bright or c.polar_night.origin,
      bg = utils.make_global_bg(options.transparent),
    }, -- Separators between window splits.
    Folded = { fg = c.frost.ice, bg = c.polar_night.bright }, -- line used for closed folds
    ...
    LineNr = { fg = c.polar_night.brightest, bg = utils.make_global_bg(options.transparent) }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    ...
    IncSearch = { fg = c.polar_night.origin, bg = c.aurora.yellow },
    Search = options.search.theme == "vscode" and { fg = c.none, bg = utils.darken(c.frost.ice, 0.2) }
      or { fg = c.polar_night.origin, bg = c.frost.ice }, -- Last search pattern highlighting (see 'hlsearch').  Also used for similar items that need to stand out.
    ...
    SpellBad = { sp = c.aurora.red }, -- Word that is not recognized by the spellchecker. |spell| Combined with the highlighting used otherwise.
    SpellCap = { sp = c.aurora.yellow }, -- Word that should start with a capital. |spell| Combined with the highlighting used otherwise.
    SpellLocal = { sp = c.snow_storm.brighter }, -- Word that is recognized by the spellchecker as one that is used in another region. |spell| Combined with the highlighting used otherwise.
    SpellRare = { fg = c.snow_storm.brightest, bg = global_bg, sp = c.snow_storm.brightest }, -- Word that is recognized by the spellchecker as one that is hardly ever used.  |spell| Combined with the highlighting used otherwise.
    ...
    WarningMsg = { fg = c.polar_night.origin, bg = utils.make_global_bg(options.transparent) }, -- warning messages
